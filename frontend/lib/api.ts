...
export type PortfolioComparison = { comparison_id: string; request: { refinery: string; required_volume_bpd: number; alternative_route_capacity_ratio: number; spr_bridge_opt_in?: boolean; government_authorization_assumed_for_scenario?: boolean; spr_bridge_duration_days?: number }; portfolios: Portfolio[] };
...
export type CaseEvidenceLedgerEntry = { evidence_id: string; label: string; source_status: string; validation_status: "UNVERIFIED_USER_INPUT" | "MODEL_PROPOSAL" | "SEEDED_CONSTRAINT" | "FIXTURE_CONTEXT"; observed_at: string | null; freshness_confidence: number; validation_confidence: number; requires_analyst_review: boolean; detail: string };
export type EvidenceValidationSummary = { status: "REQUIRES_ANALYST_REVIEW" | "SCENARIO_CONTEXT_ONLY"; freshness_confidence: number; validation_confidence: number; independently_verified_evidence_count: number; unverified_evidence_count: number; requires_analyst_review: true; rationale: string; next_actions: string[] };
export type CaseReplayContext = { status: "LINKED" | "NOT_AVAILABLE"; replay_id: string | null; title: string | null; source_status: string; evidence_item_count: number; disclaimer: string; note: string };
export type CaseDecisionClockContext = { status: "LINKED" | "NOT_AVAILABLE"; clock_id: string | null; source_status: string; decision_lead_time_hours: number | null; last_responsible_action_offset_hours: number | null; disclaimer: string; note: string };
export type CanonicalIntelligenceCase = { case_id: string; status: "LOCAL_SCENARIO"; source_status: "Simulated"; evidence_ledger: CaseEvidenceLedgerEntry[]; evidence_validation: EvidenceValidationSummary; replay_context: CaseReplayContext; decision_clock_context: CaseDecisionClockContext; national_capacity_request: MultiRefineryRequest; limitations: string[] };
export type WorkflowProposal = { workflow_id: string; status: "AWAITING_ANALYST_CONFIRMATION"; created_at: string; refinery: string; required_volume_bpd: number; processed_signal: ProcessedSignal; proposed_assumptions: WorkflowAssumptions; agent_trace: AgentTraceEntry[]; case_context?: CanonicalIntelligenceCase | null };
export type DecisionSafetyGateSource = { label: string; kind: "EVIDENCE" | "CONSTRAINT" | "ASSUMPTION"; source_status: string; detail: string };
export type DecisionSafetyGateCheck = { check_id: "EXTRACTION_CONFIDENCE" | "CORRIDOR_RESOLUTION" | "SOURCE_PROVENANCE" | "SUPPLY_FEASIBILITY" | "NATIONAL_CAPACITY"; state: "PASS" | "WARNING" | "BLOCKER"; summary: string; sources: DecisionSafetyGateSource[]; next_actions: string[] };
...
export type MultiRefineryDemandLine = { refinery: string; required_volume_bpd: number; source_status?: "User-entered" | "Simulated" };
...
export type WorkflowProposalPayload = { text: string; refinery: string; required_volume_bpd: number; national_demand_lines?: MultiRefineryDemandLine[] };
export const proposeWorkflow = (payload: WorkflowProposalPayload) => request<WorkflowProposal>("/api/v1/workflows/propose", { method: "POST", body: JSON.stringify(payload) });
